...
contract ReputationSystem is Ownable, ReentrancyGuard {
    ...
    constructor() {
        ...
        actionTypeWeights["inaccurate_vote"] = 50; // Negative impact will be applied as negative value
    ...
    function updateReputation(
        ...
        int256 scoreChange = _isPositive
            ? int256(baseWeight.mul(uint256(_scoreModifier)).div(100))
            : -int256(baseWeight.mul(uint256(_scoreModifier)).div(100));

        // Apply the reputation change
        if (scoreChange > 0) {
            userRep.totalScore = userRep.totalScore.add(uint256(scoreChange));
            ...
            if (userRep.totalScore > decrease) {
                userRep.totalScore = userRep.totalScore.sub(decrease);
            } else {
    ...
    function submitAIValidation(
        ...
        if (_validationResult && _confidenceScore >= 80) {
            ...
            uint256 bonus = AI_VALIDATION_WEIGHT.mul(_confidenceScore).div(100);
            userRep.aiValidationScore = userRep.aiValidationScore.add(bonus);
            userRep.totalScore = userRep.totalScore.add(bonus);
            ...
            uint256 penalty = AI_VALIDATION_WEIGHT.mul(_confidenceScore).div(
                200
            ); // Half penalty
            if (userRep.aiValidationScore > penalty) {
                userRep.aiValidationScore = userRep.aiValidationScore.sub(
                    penalty
                );
            } else {
            ...
            if (userRep.totalScore > penalty) {
                userRep.totalScore = userRep.totalScore.sub(penalty);
            } else {
                userRep.totalScore = 0;
            }
        }
    ...
    function endorseUser(address _user) external {
        ...
        uint256 endorsementValue = userReputations[msg.sender].totalScore.div(
            100
        ); // 1% of endorser's reputation
        endorsements[msg.sender][_user] = endorsementValue;
        endorsedBy[_user].push(msg.sender);

        // Apply endorsement bonus
        UserReputation storage userRep = userReputations[_user];
        userRep.communityContribution = userRep.communityContribution.add(
            endorsementValue
        );
        userRep.totalScore = userRep.totalScore.add(endorsementValue.div(2)); // 50% of endorsement value
    ...
    function _updateSpecificMetrics(
        ...
        if (
            ...
            userRep.votingAccuracy = userRep.votingAccuracy.add(
            ...
            if (userRep.votingAccuracy > uint256(-_scoreModifier)) {
                userRep.votingAccuracy = userRep.votingAccuracy.sub(
                    uint256(-_scoreModifier)
                );
            } else {
    ...
    function _applyTimeDecay(address _user) internal {
        ...
        uint256 timeElapsed = block.timestamp.sub(userRep.lastUpdateTimestamp);
        uint256 monthsElapsed = timeElapsed.div(30 days);

        if (monthsElapsed > 0) {
            uint256 decayAmount = userRep
                .totalScore
                .mul(DECAY_RATE)
                .mul(monthsElapsed)
                .div(1000);
            if (userRep.totalScore > decayAmount) {
                userRep.totalScore = userRep.totalScore.sub(decayAmount);
            } else {
                userRep.totalScore = 0;
            }
            userRep.timeBasedDecay = userRep.timeBasedDecay.add(decayAmount);
        }
    }
